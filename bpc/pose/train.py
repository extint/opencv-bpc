...
from bpc.utils.data_utils import BOPSingleObjRGBDDataset, bop_collate_fn
from bpc.pose.models.gdr_net import GDRNet_RGBD
...
def find_scenes(root_dir):
    ...
    train_pbr_dir = os.path.join(root_dir, "") # TRAINIG ON VAL ITSELF
...
def main():
    ...
    train_ds_fixed = BOPSingleObjRGBDDataset(
    ...
    train_ds_aug = BOPSingleObjRGBDDataset(
        root_dir=args.root_dir,
        scene_ids=scene_ids,
        cam_ids=["cam1", "cam2", "cam3"],
        target_obj_id=args.target_obj_id,
    ...
    val_ds = BOPSingleObjRGBDDataset(
        root_dir=args.root_dir,
        scene_ids=scene_ids,
        cam_ids=["cam1", "cam2", "cam3"],
        target_obj_id=args.target_obj_id,
    ...
    model = GDRNet_RGBD(num_keypoints=8, pretrained=True).to(device)
